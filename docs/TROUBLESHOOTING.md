...
- Grant **Accessibility** permission: System Settings → Privacy & Security →
  Accessibility → enable FreshLock. Preferences → Advanced shows the current status.
...
`SMAppService` requires the app to live in `/Applications`. Move `FreshLock.app`
...
FreshLock is notification-driven and should idle near 0% CPU. If you see sustained
usage, enable **Developer Mode** (Preferences → Advanced) and check the unified
log:

```bash
log stream --predicate 'subsystem == "gg.tame.freshlock"' --level debug
```
