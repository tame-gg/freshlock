...
//  FreshLock
//
//  The SwiftUI entry point. FreshLock is a menu-bar utility whose menu-bar item,
...
enum MenuBarPreference {
    static let key = "gg.tame.freshlock.showMenuBarIcon"
}

@main
struct FreshLockApp: App {
    /// Bootstraps the locking engine, the menu-bar item and window management.
    @NSApplicationDelegateAdaptor(AppDelegate.self) private var appDelegate

    var body: some Scene {
        // A SwiftUI `App` must declare at least one scene, but FreshLock's real
        ...
        // showing anything.
        Window("FreshLock", id: "root") { EmptyView() }
