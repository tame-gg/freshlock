...
//  notifications. This is the public, notification-driven (zero-polling) way to
//  observe app lifecycle on macOS — no private APIs, no CPU spent spinning.
//
...
//  a pre-launch veto. AppLock therefore reacts as fast as the OS allows — on
...
final class AppMonitorService: AppMonitorServiceProtocol {
    ...
    func start() {
        ...
        observers.append(center.addObserver(
            ...
            guard let info = Self.info(from: note) else { return }
            MainActor.assumeIsolated {
                self?.subject.send(.terminated(bundleID: info.bundleID))
            }
        })
