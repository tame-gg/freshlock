...
//  FreshLockCore
//
//  The one and only place FreshLock talks to biometrics. We use Apple's
//  `LocalAuthentication` framework and its *native* evaluation UI — FreshLock
...
public final class LocalAuthenticationService: AuthenticationServiceProtocol {
    ...
    /// In-flight evaluation contexts, keyed so concurrent prompts (e.g. Unlock
    /// All vs a single-app prompt) don't clobber each other, and `cancel()` can
    /// tear down every open system sheet.
    private var activeContexts: [UUID: LAContext] = [:]
    ...
    public func authenticate(reason: String) async -> AuthResult {
        // Fresh context per attempt. `LockCoordinator.armHostForTouchID` activates
        // the host so this sheet can be key, then immediately unhides the
        // protected app (accessory activation would otherwise hide it).
        let id = UUID()
        let context = LAContext()
        context.localizedCancelTitle = "Cancel"
        activeContexts[id] = context
        defer { activeContexts[id] = nil }
        ...
        do {
            ...
            let mapped = Self.mapError(laError as NSError)
            if case .systemCancel = mapped {
                Log.auth.notice("Authentication interrupted by systemCancel")
            }
            return .failure(mapped)
        } catch {
